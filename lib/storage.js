...
storage.crashSafeWriteFile = function (filename, fileMode, data, cb) {
  ...
  , function (cb) {
      storage.exists(filename, function (exists) {
        if (exists) {
          storage.flushToStorage({ filename: filename, fileMode: fileMode }, function (err) { return cb(err); });
        } else {
          return cb();
        }
      });
  ...
  , async.apply(storage.flushToStorage, { filename: tempFilename, fileMode: fileMode })
  , function (cb) {
 ...
 */
storage.ensureDatafileIntegrity = function (filename, fileMode, callback) {
  ...
  storage.exists(filename, function (filenameExists) {
    ...
    storage.exists(tempFilename, function (oldFilenameExists) {
      ...
      if (!oldFilenameExists) {
        return storage.writeFile(filename, '', { mode: fileMode, encoding: 'utf8' }, function (err) { callback(err); });
      }
